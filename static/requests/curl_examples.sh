...
## POST
curl -H "Content-type: xml" -X POST -d@./static/requests/crop_color_sample_czml_post.xml $server # -o ./outputs/crop_color_sample_czml_post.czml && cat ./outputs/crop_color_sample_czml_post.czml
curl -H "Content-type: xml" -X POST -d@./static/requests/crop_color_sample_post.xml $server -o outputs/crop_color_sample.tif
curl -H "Content-type: xml" -X POST -d@./static/requests/crop_color_post.xml http://localhost/wps -o outputs/crop_color.tif

curl -H "Content-type: xml" -X POST -d@./static/requests/viewshed_post.xml $server
